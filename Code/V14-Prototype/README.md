/*
==========================================
DormMate V13 Prototype

Subsystems:
- RTC Clock System
- Alarm System
- Reminder System
- Academic Planner
- Class Scheduler
- Next Class Engine
- Smart Reminder Escalation

Current Version:
V14 Prototype

Next Milestone:
CODE CLEAN UP
ESP32 MIGRATION
==========================================
*/

#include <LiquidCrystal.h>
#include <AlashIRControlRX.h>
#include <Wire.h>
#include <RTClib.h>

RTC_DS3231 rtc;
int H = 0;
int M = 0;
int S = 0;

int rtcDay = 0;
int rtcMonth = 0;
int rtcYear = 0;

// ---------------- LCD ----------------
const int rs = 12, en = 11, d4 = 5, d5 = 4, d6 = 3, d7 = 2;
LiquidCrystal lcd(rs, en, d4, d5, d6, d7);

// ---------------- NIGHT LIGHT ----------------
const int YLED = 7;
const int LIGHT = A0;

int val = 0;

// ---------------- TIMER ----------------
const unsigned long SECONDS = 60000;
unsigned long previousTime = 0;

//int H = 10;
//int M = 15;
// TIME MODE VARIABLES
bool is24hours = true;
int timeModeRow = 0;
// -------- ALARM VARIABLES --------
int alarmHour = 7;
int alarmMinute = 30;

bool alarmEnabled = false;
bool alarmRinging = false;

bool alarmTriggered = false;

int alarmEditMode = 0; // 0 = hour, 1 = minute
// ---------- Alarm --------------------
# define NOTE_C 262// HZ
# define NOTE_E 330// HZ
# define NOTE_G 392// HZ
# define NOTE_A 440 // HZ

const int speaker = 6; //SPEAKER on pin 12
//--------------Reminders--------------
// ---------- REMINDERS ----------
int reminderHour0 = 0;
int reminderMinute0 = 0;
int reminderHour1 = 0;
int reminderMinute1 = 0;
int reminderHour2 = 0;
int reminderMinute2 = 0;
int reminderHour3 = 0;
int reminderMinute3 = 0;
int reminderHour4 = 0;
int reminderMinute4 = 0;

int currentReminderMinute =0;// the minute if the current reminder
int currentReminder = 0;// select one of the reminders in the array


bool reminderEnabled = false;
bool reminderTriggered = false;
bool reminderRinging = false;

// reminder menu navigation
int r = 0; // Reminder top row
int c = 1;// reminder Bottom row

String Reminderd[] =
{
  "MATH HW",
  "HUM PAPER",
  "SPORT",
  "MEETING",
  "PRACTICE"
};

// open reminder screens
bool rem0Open = false;
bool rem1Open = false;
bool rem2Open = false;
bool rem3Open = false;
bool rem4Open = false;

// ---------------- IR REMOTE ----------------
AlashIRControlRX irReceiver(8);

// ---------------- SCREEN CONTROL ----------------
int screen = 0;

// ---------------- ENCOURAGEMENTS ----------------
String messages[] = {
  "Keep Going!",
  "You Got This!",
  "Stay Focused!",
  "Future Engineer!",
  "Never Give Up!"
};
/* Menu variables
array has all the items displayed in the menu  */
String menud[]={
  "Time",//0
  "Sensor value",//1
  "motivation message",//2
  "Set time",//3
  "Time mode",//4
  "Alarm"     ,//5
  "REMINDER",//6
  "CLASS SCHEDULE",//7
  "SetWeekDay",//8
};
int x=0 ;// menu navigation upper 
int y=1; // menu navigation lower
//control arrow
int selectedRow = 0;
/*Screen 0 = menu
  screen 1 = clock
  screen 2 = sensor value
  screen3 = motivation
  screen 4 = set time
  screen 5 = time mode
  screen 6 = alarm
  screen 7 = alarm screen 
  screen 8 = Reminder
  screen 9 = ----
  screen 10 = rem0
  screen 11 = reminder screen
  screen 12 = rem1
  screen 13 = rem2
  screen 14 = rem3
  screen 15 = rem4  
  screen 16 = schedule my classes

*/
// CLASSES SCHEDULE
String classes[]=
{
  "CALC2",        //0 SCREEN 17
  "PHYS211",     //1 screen 18
  "ECON",       // 2 screen 19
  "ASSEMBLY",  // 3 screen 20
  "STATIC",   // 4 screen 21
};
int classesTop =0;
int classesBottom = 1;

// -------- CALC2 --------
int calcHour = 9;
int calcMinute = 0;
//-------- phys211 ---------
int physHour = 12;
int physMinute = 0;

//------------ econ ------------
int econHour = 15;
int econMinute = 0;

//--------assembly --------
int assemblyHour = 9;
int assemblyMinute = 0;

//---------- static --------
int staticHour = 12;
int staticMinute = 0;

//--------- ENABLE CLASS ------------
bool econEnabled = false;
bool physEnabled = false;
bool calcEnabled = false;
bool assemblyEnabled = false;
bool staticEnabled = false ;


/*MWF = Monday, wednesday, friday 
  TT  = Tuesday, Thursday       */ 
String calcDays[] =
{
   "MWF",
   "TT"
};
int econMeetingDay = 0;
int physMeetingDay = 0;
int calcMeetingDay = 0;
int assemblyMeetingDay = 0;
int staticMeetingDay = 0;

// --------------- SCREEN folder  ---------------
const int menu_screen       = 0;
const int clock_screen      = 1;
const int sensorValue_screen= 2;
const int motivation_screen = 3;
const int setTime_screen    = 4;
const int timeMode_screen   = 5;
const int alarm_screen      = 6;
const int alarmScreen_screen= 7;
const int Reminder_screen   = 8;
const int ReminderScreen_screen =11;
const int scheduleMyClasses_screen = 16;
const int calc_screen       = 17;
const int phys_screen       = 18;
const int econ_screen       = 19;
const int assembly_screen   = 20;
const int statics_screen    = 21;
const int setweekday_screen = 22;
//----------------------------------------------------

//-------------- WeekDays------------------
String weekDAYS[]={
  "Monday",//0
  "Tuesday",//1
  "Wednesday",//2
  "Thursday",//3
  "Friday",//4
  "Saturday",//5
  "Sunday",//6
};

int currentWeekDay = 0;
// ------------- next class -----------
// -------- NEXT CLASS --------

String nextClass = "NONE";


int nextClassHour = 0;
int nextClassMinute = 0;

bool showClassReminder = false;

String reminderClass = "";

int reminderMinutes = 0;
bool reminder30Sent = false;
bool reminder15Sent = false;
bool reminder5Sent = false;

void setup()
{
  lcd.begin(16, 2);

  Serial.begin(9600);

  pinMode(YLED, OUTPUT);
  pinMode(speaker, OUTPUT);// speaker

  irReceiver.begin();

  randomSeed(analogRead(A1));

    if (!rtc.begin())
    {
        lcd.print("RTC NOT FOUND");
        while(1);
    }

    // ONLY USE ONCE TO SET RTC
    // rtc.adjust(DateTime(F(__DATE__), F(__TIME__)));
    lcd.clear();
    RTC_Time();

}
    void RTC_Time()
{
    DateTime now = rtc.now();

    H = now.hour();
    M = now.minute();
    S = now.second();

    rtcDay   = now.day();
    rtcMonth = now.month();
    rtcYear  = now.year();
}
void loop()
{
    // -------- CORE SYSTEMS --------
    autoLight();
    RTC_Time();

    alarmCheck();
    RemoteControl();

    // -------- SCREENS --------
    screenManager();

    // -------- REMINDERS --------
    Remindercheck();

    // -------- SMART LOGIC --------
    NextClass();

    delay(200);
}



/*=======================================================
-SCREEN MANAGER
- Contains all screens displayed by DormMate
=======================================================*/
void screenManager()
{
    // -------- SCREEN 0 --------
    menu();

    // -------- SCREEN 1 --------
    clock();

    // -------- SCREEN 2 --------
    sensorvalue();

    // -------- SCREEN 3 --------
    motivation();

    // -------- SCREEN 4 --------
   // setTime();

    // -------- SCREEN 5 --------
    //timeMode();

    // -------- SCREEN 6 --------
    alarm();

    // -------- SCREEN 7 --------
    alarmScreen();

    // -------- SCREEN 8 --------
    Reminder();

    // -------- SCREEN 9 --------
    ReminderScreen();

    // -------- SCREEN 10 --------
    reminder0();

    // -------- SCREEN 11 --------
    reminder1();

    // -------- SCREEN 12 --------
    reminder2();

    // -------- SCREEN 13 --------
    reminder3();

    // -------- SCREEN 14 --------
    reminder4();

    // -------- SCREEN 16 --------
    ScheduleMyClasses();

     classesNames();// actual classes name

    // -------- SCREEN 22 --------
    setWeekDay();
}
/* ========================================================
- AUTOLIGHT 
- control wether light on or off
============================================================ */
void autoLight(){
    // -------- LIGHT SENSOR --------
  val = analogRead(LIGHT);

  // -------- AUTO NIGHT LIGHT --------
  if (val < 300)
  {
    digitalWrite(YLED, HIGH);
  }
  else
  {
    digitalWrite(YLED, LOW);
  }
}

/* ========================================================
- VirtualTimeCount 
- Calculate the time using millis()
============================================================ */

/* ========================================================
- RemoteControl 
- Contain hexadecimal values of used buttons and some unused
============================================================ */
void RemoteControl(){
  // -------- IR REMOTE --------
  if (irReceiver.check())
  {
    Serial.println(irReceiver.data, HEX);

    // BUTTON 0
    if(irReceiver.data == 0xFF6897)
    {
      screen = 0;
    }
    // BUTTON 1
    if (irReceiver.data == 0xFF30CF)
    {
      //screen = 1;
    }

    // BUTTON 2
    if (irReceiver.data == 0xFF18E7)
    {
      //screen = 2;
    }

    // BUTTON 3
    if (irReceiver.data == 0xFF7A85)
    {
      //screen = 3;
    }
    
    // BUTTON 4
    if (irReceiver.data == 0xFF10EF)
    {
     // screen = 4;
    }
  }
}
/* ========================================================
- Menu screen  
- Displays the items that are in the menu 
- allow selection of items 
- trasnfer the user from the Mneu screen (Screen = 0) to the selected Item screen
============================================================ */

void menu()
{
    if(screen == menu_screen)
    {
        lcd.clear();

        // ---------- Draw Arrow ----------
        if(selectedRow == 0)
        {
            lcd.setCursor(0,0);
            lcd.print(">");

            lcd.setCursor(0,1);
            lcd.print(" ");
        }
        else
        {
            lcd.setCursor(0,0);
            lcd.print(" ");

            lcd.setCursor(0,1);
            lcd.print(">");
        }

        // ---------- Display Menu ----------
        lcd.setCursor(2,0);
        lcd.print(menud[x]);

        lcd.setCursor(2,1);
        lcd.print(menud[y]);

        // ---------- DOWN BUTTON ----------
        if(irReceiver.data == 0xFFE01F)
        {
            // Move arrow down first
            if(selectedRow == 0)
            {
                selectedRow = 1;
            }
            else
            {
                // Scroll menu
                if(y < 8)
                {
                    x++;
                    y++;
                }
            }

            irReceiver.data = 0;
        }

        // ---------- UP BUTTON ----------
        if(irReceiver.data == 0xFF906F)
        {
            // Move arrow up first
            if(selectedRow == 1)
            {
                selectedRow = 0;
            }
            else
            {
                // Scroll upward
                if(x > 0)
                {
                    x--;
                    y--;
                }
            }

            irReceiver.data = 0;
        }

        // ---------- PLAY / OK BUTTON ----------
        if(irReceiver.data == 0xFF02FD)
        {
            int selectedItem;

            // Determine selected item
            if(selectedRow == 0)
            {
                selectedItem = x;
            }
            else
            {
                selectedItem = y;
            }

            // Open screen
            switch(selectedItem)
            {
                case 0:
                    screen = 1;
                    break;

                case 1:
                    screen = 2;
                    break;

                case 2:
                    screen = 3;
                    break;

                case 3:
                    screen = 4;
                    break;

                case 4:
                    screen = 5;
                    break;
                case 5:
                    screen = 6;
                    break;  
                case 6:
                    screen = 8;
                    break;
                case 7:
                    screen = 16;
                    break;
                case 8:
                    screen = 22;
                    break;              
            }

            irReceiver.data = 0;
        }
    }
}

/* ==========================================
- CLOCK SCREEN
- Displays current time and weekday
- Screen 1
 ==========================================*/
void clock()
{
    if(screen == clock_screen)
    {
    DateTime now = rtc.now();

    int H = now.hour();
    int M = now.minute();
    int S = now.second();

    lcd.clear();

    // ---------- TOP ROW ----------
    lcd.setCursor(0,0);
    lcd.print("Time ");

    if(H < 10) lcd.print("0");
    lcd.print(H);

    lcd.print(":");

    if(M < 10) lcd.print("0");
    lcd.print(M);

    lcd.print(":");

    if(S < 10) lcd.print("0");
    lcd.print(S);

    // ---------- BOTTOM ROW ----------
    lcd.setCursor(0,1);

    if(now.month() < 10) lcd.print("0");
    lcd.print(now.month());

    lcd.print("/");

    if(now.day() < 10) lcd.print("0");
    lcd.print(now.day());

    lcd.print("/");

    lcd.print(now.year());

    Serial.print(H);
    Serial.print(":");
    Serial.println(M);

    delay(1000);
    }
}
/* ========================================================
- sensorvalue screen 
- Display value of room light
- screen 2
============================================================ */
void sensorvalue(){
    if (screen == sensorValue_screen)
  {
    lcd.clear();

    lcd.setCursor(0, 0);
    lcd.print("Sensor Value");

    lcd.setCursor(0, 1);
    lcd.print(val);
  }
}

void motivation(){
    if (screen == motivation_screen)
  {
    lcd.clear();

    int msg = random(0, 5);

    lcd.setCursor(0, 0);
    lcd.print("Message:");

    lcd.setCursor(0, 1);
    lcd.print(messages[msg]);

    delay(2000);

   // screen = 1;
  }
}

/* ========================================================
- setTime screen 
- Allows the user to set the time manually with remote 
- screen 4
============================================================ */

/* ========================================================
- timeMode screen 
- Allows the user to select the time mode 12h/24h with remote 
- screen 5
============================================================ */

/* ========================================================
- alarm screen 
- Allows the user to set an alarm 
- screen 6
============================================================ */
void alarm()
{
    if(screen == alarm_screen)
    {
        lcd.clear();

        lcd.setCursor(0,0);
        lcd.print("Alarm");

        // display alarm time
        lcd.setCursor(0,1);

        if(alarmHour < 10) lcd.print("0");
        lcd.print(alarmHour);

        lcd.print(":");

        if(alarmMinute < 10) lcd.print("0");
        lcd.print(alarmMinute);

        lcd.print(" ");

        if(alarmEnabled)
        {
            lcd.print("ON");
        }
        else
        {
            lcd.print("OFF");
        }

        // -------- HOUR UP --------
        if(irReceiver.data == 0xFF906F)
        {
            alarmHour++;

            if(alarmHour == 24)
            {
                alarmHour = 0;
            }

            irReceiver.data = 0;
        }

        // -------- HOUR DOWN --------
        if(irReceiver.data == 0xFFE01F)
        {
            alarmHour--;

            if(alarmHour < 0)
            {
                alarmHour = 23;
            }

            irReceiver.data = 0;
        }

        // -------- MINUTE UP --------
        if(irReceiver.data == 0xFFC23D)
        {
            alarmMinute++;

            if(alarmMinute == 60)
            {
                alarmMinute = 0;
            }

            irReceiver.data = 0;
        }

        // -------- MINUTE DOWN --------
        if(irReceiver.data == 0xFF22DD)
        {
            alarmMinute--;

            if(alarmMinute < 0)
            {
                alarmMinute = 59;
            }

            irReceiver.data = 0;
        }

        // -------- OK BUTTON --------
        if(irReceiver.data == 0xFF02FD)
        {
            alarmEnabled = !alarmEnabled;
            irReceiver.data = 0;
        }
    }
}
/* ========================================================
- alarm CHECK
- Check if the alarm was triggered
- emit the sound of the alarm
============================================================ */
void alarmCheck()
{
    // -------- Trigger alarm ONCE --------
    if(alarmEnabled &&
       !alarmTriggered)
    {
        if(H == alarmHour &&
           M == alarmMinute)
        {
            alarmRinging = true;
            alarmTriggered = true;
            screen = 7;
        }
    }

    // -------- Ringing --------
    if(alarmRinging)
    {
        playTone(262,200);
        playTone(330,200);
        playTone(440,200);

        // BUTTON 1 stops alarm
        if(irReceiver.data == 0xFF30CF)
        {
            alarmRinging = false;

            screen = 1;

            irReceiver.data = 0;
        }
    }

    // -------- Reset trigger next minute --------
    if(M != alarmMinute)
    {
        alarmTriggered = false;
    }
}
/* ========================================================
- Play tone
- playTone(f,d) allows the program to emit sound with buzzer. Because of interrupt issues, we cannot use tone()

============================================================ */
void playTone(int frequency, int duration)
{
    long delayValue = 1000000 / frequency / 2;
    long cycles = frequency * duration / 1000;

    for(long i = 0; i < cycles; i++)
    {
        digitalWrite(speaker, HIGH);
        delayMicroseconds(delayValue);

        digitalWrite(speaker, LOW);
        delayMicroseconds(delayValue);
    }
}
/* ========================================================
- alarm display screen 
- Displays the alarm and give option to stop the alarm by pressing 1
- screen 7
============================================================ */
void alarmScreen(){
  if(screen == alarmScreen_screen){
  lcd.clear();
  lcd.setCursor(0,0);
  lcd.print("PRESS 1(STOP)");
  }

  // JUST TRYING TO SEE IF IT WILL STOP AND RETURN TO SCREEN 1
          if(irReceiver.data ==   0xFF30CF)
        {
            alarmRinging = false;
            irReceiver.data = 0;
            screen = 1;// back to time
        }
}

/* ========================================================
- Reminder screen 
- Allows the user to select an item in the reminder menu screen
- screen 8
============================================================ */
void Reminder()
{
    if(screen == Reminder_screen)
    {
        lcd.clear();

        // -------- Draw Arrow --------
        if(selectedRow == 0)
        {
            lcd.setCursor(0,0);
            lcd.print(">");

            lcd.setCursor(0,1);
            lcd.print(" ");
        }
        else
        {
            lcd.setCursor(0,0);
            lcd.print(" ");

            lcd.setCursor(0,1);
            lcd.print(">");
        }

        // -------- Display Menu --------
        lcd.setCursor(2,0);
        lcd.print(Reminderd[r]);

        lcd.setCursor(2,1);
        lcd.print(Reminderd[c]);

        // -------- DOWN BUTTON --------
        if(irReceiver.data == 0xFFE01F)
        {
            if(selectedRow == 0)
            {
                selectedRow = 1;
            }
            else
            {
                if(c < 4)
                {
                    r++;
                    c++;
                }
            }

            irReceiver.data = 0;
        }

        // -------- UP BUTTON --------
        if(irReceiver.data == 0xFF906F)
        {
            if(selectedRow == 1)
            {
                selectedRow = 0;
            }
            else
            {
                if(r > 0)
                {
                    r--;
                    c--;
                }
            }

            irReceiver.data = 0;
        }

        // -------- OK BUTTON --------
        if(irReceiver.data == 0xFF02FD)
        {
            int selectedItem;

            if(selectedRow == 0)
            {
                selectedItem = r;
            }
            else
            {
                selectedItem = c;
            }

            switch(selectedItem)
            {
                case 0:
                    rem0Open = true;
                    screen = 10;
                    break;
                case 1: 
                    rem1Open = true;
                    screen   = 12;
                    break;    
                case 2: 
                    rem2Open = true;
                    screen   = 13;
                    break; 
                case 3: 
                    rem3Open = true;
                    screen   = 14;
                    break;  
                case 4: 
                    rem4Open = true;
                    screen   = 15;
                    break;                                                            
            }

            irReceiver.data = 0;
        }
    }
}

/* ========================================================
- reminder0 to reminder4
- Allows the user to set a reminder time for the reminder selected
- screen 10 (reminder0)
- screen 12 (reminder1)
- screen 13 (reminder2)
- screen 14 (reminder3)
- screen 15 (reminder4)
============================================================ */
void reminder0()
{
    if(rem0Open && screen == 10)
    {
        lcd.clear();

        lcd.setCursor(0,0);
        lcd.print("Reminder at");

        lcd.setCursor(0,1);

        if(reminderHour0 < 10)
            lcd.print("0");

        lcd.print(reminderHour0);

        lcd.print(":");

        if(reminderMinute0 < 10)
            lcd.print("0");

        lcd.print(reminderMinute0);

        lcd.print(" ");

        if(reminderEnabled)
        {
            lcd.print("ON");
        }
        else
        {
            lcd.print("OFF");
        }

        // HOUR UP
        if(irReceiver.data == 0xFF906F)
        {
            reminderHour0++;

            if(reminderHour0 == 24)
            {
                reminderHour0 = 0;
            }

            irReceiver.data = 0;
        }

        // HOUR DOWN
        if(irReceiver.data == 0xFFE01F)
        {
            reminderHour0--;

            if(reminderHour0 < 0)
            {
                reminderHour0 = 23;
            }

            irReceiver.data = 0;
        }

        // MINUTE UP
        if(irReceiver.data == 0xFFC23D)
        {
            reminderMinute0++;

            if(reminderMinute0 == 60)
            {
                reminderMinute0 = 0;
            }

            irReceiver.data = 0;
        }

        // MINUTE DOWN
        if(irReceiver.data == 0xFF22DD)
        {
            reminderMinute0--;

            if(reminderMinute0 < 0)
            {
                reminderMinute0 = 59;
            }

            irReceiver.data = 0;
        }

        // OK BUTTON
        if(irReceiver.data == 0xFF02FD)
        {
            reminderEnabled = !reminderEnabled;
            irReceiver.data = 0;
        }

        // BUTTON 0 returns menu
        if(irReceiver.data == 0xFF6897)
        {
            rem0Open = false;
            screen = 8;

            irReceiver.data = 0;
        }
    }
}

void reminder1()
{
    if(rem1Open && screen == 12)
    {
        lcd.clear();

        lcd.setCursor(0,0);
        lcd.print("Reminder at");

        lcd.setCursor(0,1);

        if(reminderHour1 < 10)
            lcd.print("0");

        lcd.print(reminderHour1);

        lcd.print(":");

        if(reminderMinute1 < 10)
            lcd.print("0");

        lcd.print(reminderMinute1);

        lcd.print(" ");

        if(reminderEnabled)
        {
            lcd.print("ON");
        }
        else
        {
            lcd.print("OFF");
        }

        // HOUR UP
        if(irReceiver.data == 0xFF906F)
        {
            reminderHour1++;

            if(reminderHour1 == 24)
            {
                reminderHour1 = 0;
            }

            irReceiver.data = 0;
        }

        // HOUR DOWN
        if(irReceiver.data == 0xFFE01F)
        {
            reminderHour1--;

            if(reminderHour1 < 0)
            {
                reminderHour1 = 23;
            }

            irReceiver.data = 0;
        }

        // MINUTE UP
        if(irReceiver.data == 0xFFC23D)
        {
            reminderMinute1++;

            if(reminderMinute1 == 60)
            {
                reminderMinute1 = 0;
            }

            irReceiver.data = 0;
        }

        // MINUTE DOWN
        if(irReceiver.data == 0xFF22DD)
        {
            reminderMinute1--;

            if(reminderMinute1 < 0)
            {
                reminderMinute1 = 59;
            }

            irReceiver.data = 0;
        }

        // OK BUTTON
        if(irReceiver.data == 0xFF02FD)
        {
            reminderEnabled = !reminderEnabled;
            irReceiver.data = 0;
        }

        // BUTTON 0 returns menu
        if(irReceiver.data == 0xFF6897)
        {
            rem1Open = false;
            screen = 8;

            irReceiver.data = 0;
        }
    }
}
void reminder2()
{
    if(rem2Open && screen == 13)
    {
        lcd.clear();

        lcd.setCursor(0,0);
        lcd.print("Reminder at");

        lcd.setCursor(0,1);

        if(reminderHour2 < 10)
            lcd.print("0");

        lcd.print(reminderHour2);

        lcd.print(":");

        if(reminderMinute2 < 10)
            lcd.print("0");

        lcd.print(reminderMinute2);

        lcd.print(" ");

        if(reminderEnabled)
        {
            lcd.print("ON");
        }
        else
        {
            lcd.print("OFF");
        }

        // HOUR UP
        if(irReceiver.data == 0xFF906F)
        {
            reminderHour2++;

            if(reminderHour2 == 24)
            {
                reminderHour2 = 0;
            }

            irReceiver.data = 0;
        }

        // HOUR DOWN
        if(irReceiver.data == 0xFFE01F)
        {
            reminderHour2--;

            if(reminderHour2 < 0)
            {
                reminderHour2 = 23;
            }

            irReceiver.data = 0;
        }

        // MINUTE UP
        if(irReceiver.data == 0xFFC23D)
        {
            reminderMinute2++;

            if(reminderMinute2 == 60)
            {
                reminderMinute2 = 0;
            }

            irReceiver.data = 0;
        }

        // MINUTE DOWN
        if(irReceiver.data == 0xFF22DD)
        {
            reminderMinute2--;

            if(reminderMinute2 < 0)
            {
                reminderMinute2 = 59;
            }

            irReceiver.data = 0;
        }

        // OK BUTTON
        if(irReceiver.data == 0xFF02FD)
        {
            reminderEnabled = !reminderEnabled;
            irReceiver.data = 0;
        }

        // BUTTON 0 returns menu
        if(irReceiver.data == 0xFF6897)
        {
            rem2Open = false;
            screen = 8;

            irReceiver.data = 0;
        }
    }
}
void reminder3()
{
    if(rem3Open && screen == 14)
    {
        lcd.clear();

        lcd.setCursor(0,0);
        lcd.print("Reminder at");

        lcd.setCursor(0,1);

        if(reminderHour3 < 10)
            lcd.print("0");

        lcd.print(reminderHour3);

        lcd.print(":");

        if(reminderMinute3 < 10)
            lcd.print("0");

        lcd.print(reminderMinute3);

        lcd.print(" ");

        if(reminderEnabled)
        {
            lcd.print("ON");
        }
        else
        {
            lcd.print("OFF");
        }

        // HOUR UP
        if(irReceiver.data == 0xFF906F)
        {
            reminderHour3++;

            if(reminderHour3 == 24)
            {
                reminderHour3 = 0;
            }

            irReceiver.data = 0;
        }

        // HOUR DOWN
        if(irReceiver.data == 0xFFE01F)
        {
            reminderHour3--;

            if(reminderHour3 < 0)
            {
                reminderHour3 = 23;
            }

            irReceiver.data = 0;
        }

        // MINUTE UP
        if(irReceiver.data == 0xFFC23D)
        {
            reminderMinute3++;

            if(reminderMinute3 == 60)
            {
                reminderMinute3 = 0;
            }

            irReceiver.data = 0;
        }

        // MINUTE DOWN
        if(irReceiver.data == 0xFF22DD)
        {
            reminderMinute3--;

            if(reminderMinute3 < 0)
            {
                reminderMinute3 = 59;
            }

            irReceiver.data = 0;
        }

        // OK BUTTON
        if(irReceiver.data == 0xFF02FD)
        {
            reminderEnabled = !reminderEnabled;
            irReceiver.data = 0;
        }

        // BUTTON 0 returns menu
        if(irReceiver.data == 0xFF6897)
        {
            rem3Open = false;
            screen = 8;

            irReceiver.data = 0;
        }
    }
}

void reminder4()
{
    if(rem4Open && screen == 15)
    {
        lcd.clear();

        lcd.setCursor(0,0);
        lcd.print("Reminder at");

        lcd.setCursor(0,1);

        if(reminderHour4 < 10)
            lcd.print("0");

        lcd.print(reminderHour4);

        lcd.print(":");

        if(reminderMinute4 < 10)
            lcd.print("0");

        lcd.print(reminderMinute4);

        lcd.print(" ");

        if(reminderEnabled)
        {
            lcd.print("ON");
        }
        else
        {
            lcd.print("OFF");
        }

        // HOUR UP
        if(irReceiver.data == 0xFF906F)
        {
            reminderHour4++;

            if(reminderHour4 == 24)
            {
                reminderHour4 = 0;
            }

            irReceiver.data = 0;
        }

        // HOUR DOWN
        if(irReceiver.data == 0xFFE01F)
        {
            reminderHour4--;

            if(reminderHour4 < 0)
            {
                reminderHour4 = 23;
            }

            irReceiver.data = 0;
        }

        // MINUTE UP
        if(irReceiver.data == 0xFFC23D)
        {
            reminderMinute4++;

            if(reminderMinute4 == 60)
            {
                reminderMinute4 = 0;
            }

            irReceiver.data = 0;
        }

        // MINUTE DOWN
        if(irReceiver.data == 0xFF22DD)
        {
            reminderMinute4--;

            if(reminderMinute4 < 0)
            {
                reminderMinute4 = 59;
            }

            irReceiver.data = 0;
        }

        // OK BUTTON
        if(irReceiver.data == 0xFF02FD)
        {
            reminderEnabled = !reminderEnabled;
            irReceiver.data = 0;
        }

        // BUTTON 0 returns menu
        if(irReceiver.data == 0xFF6897)
        {
            rem4Open = false;
            screen = 8;

            irReceiver.data = 0;
        }
    }
}

/*=================================================
- ReminderCheck
- Checks if an item's reminder time is equal to current time.
- reminderEnabled has to be true
================================================== */
void Remindercheck()
{
    if(reminderEnabled &&
       !reminderTriggered)
    {
        if(H == reminderHour0 &&
           M == reminderMinute0)
        {
            reminderRinging = true;
            reminderTriggered = true;
            currentReminder =0;

            screen = 11;
             currentReminderMinute = reminderMinute0;
        }

        if(H == reminderHour1 &&
           M == reminderMinute1)
        {
            reminderRinging = true;
            reminderTriggered = true;
            currentReminder=1;

            screen = 11;
             currentReminderMinute = reminderMinute1;
        }

        if(H == reminderHour2 &&
           M == reminderMinute2)
        {
            reminderRinging = true;
            reminderTriggered = true;
            currentReminder=2;

            screen = 11;
             currentReminderMinute = reminderMinute2;
        }

        if(H == reminderHour3 &&
           M == reminderMinute3)
        {
            reminderRinging = true;
            reminderTriggered = true;
            currentReminder=3;

            screen = 11;
             currentReminderMinute = reminderMinute3;
        } 
        if(H == reminderHour4 &&
           M == reminderMinute4)
        {
            reminderRinging = true;
            reminderTriggered = true;
            currentReminder=4;

            screen = 11;
             currentReminderMinute = reminderMinute4;
        }                               
        
    }

    // reset next minute
     if(M != currentReminderMinute)
    {
        reminderTriggered = false;
    }
    
}
/*=================================================
- ReminderScreen
- Displays the reminder to the screen
- allows the user to exit the reminder screen by pressing 1
================================================== */
void ReminderScreen()
{
    if(screen == ReminderScreen_screen)
    {
        lcd.clear();

        lcd.setCursor(0,0);
        lcd.print("REMINDER");

        lcd.setCursor(0,1);
        lcd.print(Reminderd[currentReminder]);

        playTone(880,200);
        playTone(466,200);
        playTone(494,200);

        // BUTTON 1 stop reminder
        if(irReceiver.data == 0xFF30CF)
        {
            reminderRinging = false;
           // r = 0;
           // c = 1;
           // selectedRow = 0;

            lcd.clear();
            screen = 1;

            irReceiver.data = 0;
        }
    }
}
/*=================================================
- ScheduleMyClasses screen
- Displays the classes the user has
- Allows the user to select a class to modify the meeting days and time
- Allows the user to specify what time those classes happen
================================================== */
void ScheduleMyClasses(){
  if(screen == scheduleMyClasses_screen){
    lcd.clear();
    if(selectedRow == 0){
      lcd.setCursor(0,0);
      lcd.print(">");
      lcd.setCursor(0,1);
      lcd.print(" ");
    }
    else{
      lcd.setCursor(0,0);
      lcd.print(" ");
      lcd.setCursor(0,1);
      lcd.print(">");      
    }

    // ----------------display classes[] array---------------
    lcd.setCursor(2,0);
    lcd.print(classes[classesTop]);
    lcd.setCursor(2,1);
    lcd.print(classes[classesBottom]);

     // ------------ DOWN BUTTON ----------------    
     if(irReceiver.data == 0xFFE01F){
      
      // ---- MOVE ARROW DOWN FIRST
        if(selectedRow == 0){
          selectedRow = 1;
        }
        else{
          if(classesBottom<4){
            classesTop++;
            classesBottom++;
          }
        }
        irReceiver.data = 0;
     }

     // ----------- UP BUTTON ------------
     if(irReceiver.data == 0xFF906F){

      // ---- MOVE ARROW UP FIRST-----
        if(selectedRow == 1){
          selectedRow = 0;
        }
        else{
          if(classesTop>0){
            classesTop--;
            classesBottom--;
          }
        }
        irReceiver.data = 0;
     }

     // ---------- PLAY BUTTON ---------
     if(irReceiver.data == 0xFF02FD){

      int selectedClass;
      
      // ------- determine the class selected---------
        if(selectedRow == 0){
          selectedClass = classesTop;
        }
        else{
          selectedClass = classesBottom;
        }

        // --------- open the class ---------

        switch(selectedClass){

          case 0:
                lcd.clear();
                screen = 17;// calc2
                break;

          case 1:
                lcd.clear();
                screen = 18;// phys211
                break;

          case 2:
                lcd.clear();
                screen = 19;// Econ   
                break;

          case 3:
                lcd.clear();
                screen = 20;// assembly
                break;

          case 4:
                lcd.clear();
                screen = 21;// Static
                break;
                           
        }
        irReceiver.data = 0;
     }      
      
  }

}
/*=================================================
- Classes screen
- classes in the student schedule
================================================== */
void classesNames(){
  calc2();      // screen 17
  phys211();    // screen  18
  econ();       // screen 19
  assembly();   // screen 20
  statics();    // screen 21

}
void calc2()
{
    if(screen == calc_screen)
    {
        lcd.clear();

        // -------- TITLE --------
        lcd.setCursor(0,0);
        lcd.print("CALC2");

        // -------- DISPLAY --------
        lcd.setCursor(0,1);

        if(calcHour < 10)
            lcd.print("0");

        lcd.print(calcHour);
        lcd.print(":");

        if(calcMinute < 10)
            lcd.print("0");

        lcd.print(calcMinute);

        lcd.print(" ");

        lcd.print(calcDays[calcMeetingDay]);

        lcd.print(" ");

        if(calcEnabled)
        {
            lcd.print("ON");
        }
        else
        {
            lcd.print("OFF");
        }

        // -------- UP BUTTON --------
        // increase hour
        if(irReceiver.data == 0xFF906F)
        {
            calcHour++;

            if(calcHour == 24)
            {
                calcHour = 0;
            }

            irReceiver.data = 0;
        }

        // -------- DOWN BUTTON --------
        // decrease hour
        if(irReceiver.data == 0xFFE01F)
        {
            calcHour--;

            if(calcHour < 0)
            {
                calcHour = 23;
            }

            irReceiver.data = 0;
        }

        // -------- RIGHT BUTTON --------
        // increase minute
        if(irReceiver.data == 0xFFC23D)
        {
            calcMinute++;

            if(calcMinute == 60)
            {
                calcMinute = 0;
            }

            irReceiver.data = 0;
        }

        // ---------- Left BUTTON ------------
        //DECREASE MINUTE
        if(irReceiver.data == 0xFF22DD)
        {
            calcMinute--;

            if(calcMinute < 0)
            {
                calcMinute = 59;
            }

            irReceiver.data = 0;

        }

        // --------  BUTTON2 --------
        // change meeting days   0xFF18E7
        if(irReceiver.data == 0xFF18E7)
        {
            calcMeetingDay++;

            if(calcMeetingDay > 1)
            {
                calcMeetingDay = 0;
            }

            irReceiver.data = 0;
        }

        // -------- OK BUTTON --------
        // toggle ON/OFF
        if(irReceiver.data == 0xFF02FD)
        {
            calcEnabled = !calcEnabled;

            irReceiver.data = 0;
        }

        // -------- BUTTON 0 --------
        // back to class menu
        if(irReceiver.data == 0xFF6897)
        {
            lcd.clear();

            screen = 16;

            irReceiver.data = 0;
        }
    }
}

void phys211()
{
    if(screen == phys_screen)
    {
        lcd.clear();

        // -------- TITLE --------
        lcd.setCursor(0,0);
        lcd.print("PHYS211");

        // -------- DISPLAY --------
        lcd.setCursor(0,1);

        if(physHour < 10)
            lcd.print("0");

        lcd.print(physHour);
        lcd.print(":");

        if(physMinute < 10)
            lcd.print("0");

        lcd.print(physMinute);

        lcd.print(" ");

        lcd.print(calcDays[physMeetingDay]);

        lcd.print(" ");

        if(physEnabled)
        {
            lcd.print("ON");
        }
        else
        {
            lcd.print("OFF");
        }

        // -------- UP BUTTON --------
        // increase hour
        if(irReceiver.data == 0xFF906F)
        {
            physHour++;

            if(physHour == 24)
            {
                physHour = 0;
            }

            irReceiver.data = 0;
        }

        // -------- DOWN BUTTON --------
        // decrease hour
        if(irReceiver.data == 0xFFE01F)
        {
            physHour--;

            if(physHour < 0)
            {
                physHour = 23;
            }

            irReceiver.data = 0;
        }

        // -------- RIGHT BUTTON --------
        // increase minute
        if(irReceiver.data == 0xFFC23D)
        {
            physMinute++;

            if(physMinute == 60)
            {
                physMinute = 0;
            }

            irReceiver.data = 0;
        }

        // ---------- Left BUTTON ------------
        //DECREASE MINUTE
        if(irReceiver.data == 0xFF22DD)
        {
            physMinute--;

            if(physMinute < 0)
            {
                physMinute = 59;
            }

            irReceiver.data = 0;

        }

        // --------  BUTTON2 --------
        // change meeting days   0xFF18E7
        if(irReceiver.data == 0xFF18E7)
        {
            physMeetingDay++;

            if(physMeetingDay > 1)
            {
                physMeetingDay = 0;
            }

            irReceiver.data = 0;
        }

        // -------- OK BUTTON --------
        // toggle ON/OFF
        if(irReceiver.data == 0xFF02FD)
        {
            physEnabled = !physEnabled;

            irReceiver.data = 0;
        }

        // -------- BUTTON 0 --------
        // back to class menu
        if(irReceiver.data == 0xFF6897)
        {
            lcd.clear();

            screen = 16;

            irReceiver.data = 0;
        }
    }
}

void econ()
{
    if(screen == econ_screen)
    {
        lcd.clear();

        // -------- TITLE --------
        lcd.setCursor(0,0);
        lcd.print("ECON");

        // -------- DISPLAY --------
        lcd.setCursor(0,1);

        if(econHour < 10)
            lcd.print("0");

        lcd.print(econHour);
        lcd.print(":");

        if(econMinute < 10)
            lcd.print("0");

        lcd.print(econMinute);

        lcd.print(" ");

        lcd.print(calcDays[econMeetingDay]);

        lcd.print(" ");

        if(econEnabled)
        {
            lcd.print("ON");
        }
        else
        {
            lcd.print("OFF");
        }

        // -------- UP BUTTON --------
        // increase hour
        if(irReceiver.data == 0xFF906F)
        {
            econHour++;

            if(econHour == 24)
            {
                econHour = 0;
            }

            irReceiver.data = 0;
        }

        // -------- DOWN BUTTON --------
        // decrease hour
        if(irReceiver.data == 0xFFE01F)
        {
            econHour--;

            if(econHour < 0)
            {
                econHour = 23;
            }

            irReceiver.data = 0;
        }

        // -------- RIGHT BUTTON --------
        // increase minute
        if(irReceiver.data == 0xFFC23D)
        {
            econMinute++;

            if(econMinute == 60)
            {
                econMinute = 0;
            }

            irReceiver.data = 0;
        }

        // ---------- Left BUTTON ------------
        //DECREASE MINUTE
        if(irReceiver.data == 0xFF22DD)
        {
            econMinute--;

            if(econMinute < 0)
            {
                econMinute = 59;
            }

            irReceiver.data = 0;

        }

        // --------  BUTTON2 --------
        // change meeting days   0xFF18E7
        if(irReceiver.data == 0xFF18E7)
        {
            econMeetingDay++;

            if(econMeetingDay > 1)
            {
                econMeetingDay = 0;
            }

            irReceiver.data = 0;
        }

        // -------- OK BUTTON --------
        // toggle ON/OFF
        if(irReceiver.data == 0xFF02FD)
        {
            econEnabled = !econEnabled;

            irReceiver.data = 0;
        }

        // -------- BUTTON 0 --------
        // back to class menu
        if(irReceiver.data == 0xFF6897)
        {
            lcd.clear();

            screen = 16;

            irReceiver.data = 0;
        }
    }
}

void assembly()
{
    if(screen == assembly_screen)
    {
        lcd.clear();

        // -------- TITLE --------
        lcd.setCursor(0,0);
        lcd.print("ASSEMBLY");

        // -------- DISPLAY --------
        lcd.setCursor(0,1);

        if(assemblyHour < 10)
            lcd.print("0");

        lcd.print(assemblyHour);
        lcd.print(":");

        if(assemblyMinute < 10)
            lcd.print("0");

        lcd.print(assemblyMinute);

        lcd.print(" ");

        lcd.print(calcDays[assemblyMeetingDay]);

        lcd.print(" ");

        if(assemblyEnabled)
        {
            lcd.print("ON");
        }
        else
        {
            lcd.print("OFF");
        }

        // -------- UP BUTTON --------
        if(irReceiver.data == 0xFF906F)
        {
            assemblyHour++;

            if(assemblyHour == 24)
            {
                assemblyHour = 0;
            }

            irReceiver.data = 0;
        }

        // -------- DOWN BUTTON --------
        if(irReceiver.data == 0xFFE01F)
        {
            assemblyHour--;

            if(assemblyHour < 0)
            {
                assemblyHour = 23;
            }

            irReceiver.data = 0;
        }

        // -------- RIGHT BUTTON --------
        if(irReceiver.data == 0xFFC23D)
        {
            assemblyMinute++;

            if(assemblyMinute == 60)
            {
                assemblyMinute = 0;
            }

            irReceiver.data = 0;
        }

        // -------- LEFT BUTTON --------
        if(irReceiver.data == 0xFF22DD)
        {
            assemblyMinute--;

            if(assemblyMinute < 0)
            {
                assemblyMinute = 59;
            }

            irReceiver.data = 0;
        }

        // -------- BUTTON 2 --------
        if(irReceiver.data == 0xFF18E7)
        {
            assemblyMeetingDay++;

            if(assemblyMeetingDay > 1)
            {
                assemblyMeetingDay = 0;
            }

            irReceiver.data = 0;
        }

        // -------- OK BUTTON --------
        if(irReceiver.data == 0xFF02FD)
        {
            assemblyEnabled = !assemblyEnabled;

            irReceiver.data = 0;
        }

        // -------- BUTTON 0 --------
        if(irReceiver.data == 0xFF6897)
        {
            lcd.clear();

            screen = 16;

            irReceiver.data = 0;
        }
    }
}

void statics()
{
    if(screen == statics_screen)
    {
        lcd.clear();

        // -------- TITLE --------
        lcd.setCursor(0,0);
        lcd.print("STATIC");

        // -------- DISPLAY --------
        lcd.setCursor(0,1);

        if(staticHour < 10)
            lcd.print("0");

        lcd.print(staticHour);
        lcd.print(":");

        if(staticMinute < 10)
            lcd.print("0");

        lcd.print(staticMinute);

        lcd.print(" ");

        lcd.print(calcDays[staticMeetingDay]);

        lcd.print(" ");

        if(staticEnabled)
        {
            lcd.print("ON");
        }
        else
        {
            lcd.print("OFF");
        }

        // -------- UP BUTTON --------
        if(irReceiver.data == 0xFF906F)
        {
            staticHour++;

            if(staticHour == 24)
            {
                staticHour = 0;
            }

            irReceiver.data = 0;
        }

        // -------- DOWN BUTTON --------
        if(irReceiver.data == 0xFFE01F)
        {
            staticHour--;

            if(staticHour < 0)
            {
                staticHour = 23;
            }

            irReceiver.data = 0;
        }

        // -------- RIGHT BUTTON --------
        if(irReceiver.data == 0xFFC23D)
        {
            staticMinute++;

            if(staticMinute == 60)
            {
                staticMinute = 0;
            }

            irReceiver.data = 0;
        }

        // -------- LEFT BUTTON --------
        if(irReceiver.data == 0xFF22DD)
        {
            staticMinute--;

            if(staticMinute < 0)
            {
                staticMinute = 59;
            }

            irReceiver.data = 0;
        }

        // -------- BUTTON 2 --------
        if(irReceiver.data == 0xFF18E7)
        {
            staticMeetingDay++;

            if(staticMeetingDay > 1)
            {
                staticMeetingDay = 0;
            }

            irReceiver.data = 0;
        }

        // -------- OK BUTTON --------
        if(irReceiver.data == 0xFF02FD)
        {
            staticEnabled = !staticEnabled;

            irReceiver.data = 0;
        }

        // -------- BUTTON 0 --------
        if(irReceiver.data == 0xFF6897)
        {
            lcd.clear();

            screen = 16;

            irReceiver.data = 0;
        }
    }
}
/*============================================================================
- setWeekDay
- Allows the user to select a day of the week
- screen 22
*/
void setWeekDay()
{
    if(screen == setweekday_screen)
    {
        lcd.clear();

        lcd.setCursor(0,0);
        lcd.print("Set Day");

        lcd.setCursor(0,1);
        lcd.print(weekDAYS[currentWeekDay]);

        // -------- UP --------
        if(irReceiver.data == 0xFF906F)
        {
            currentWeekDay++;

            if(currentWeekDay > 6)
            {
                currentWeekDay = 0;
            }

            irReceiver.data = 0;
        }

        // -------- DOWN --------
        if(irReceiver.data == 0xFFE01F)
        {
            currentWeekDay--;

            if(currentWeekDay < 0)
            {
                currentWeekDay = 6;
            }

            irReceiver.data = 0;
        }

        // -------- BUTTON 0 --------
        if(irReceiver.data == 0xFF6897)
        {
            screen = 0;
            irReceiver.data = 0;
        }
    }
}
/*===============================================================
- NextClass
- Determines what is the next class by calculations
*/
void NextClass()
{
    int currentTime;

    currentTime = H * 60 + M;

    nextClass = "NONE";

    int smallestDifference = 10000;

    showClassReminder = false;

    // ==================================
    // CALC2
    // ==================================
    if(calcEnabled)
    {
        int calcTime =
        calcHour * 60 + calcMinute;

        int differenceCalc =
        calcTime - currentTime;

        if(differenceCalc > 0 &&
           differenceCalc < smallestDifference)
        {
            smallestDifference =
            differenceCalc;

            nextClass = "CALC2";

            nextClassHour =
            calcHour;

            nextClassMinute =
            calcMinute;
        }
    }

    // ==================================
    // PHYS211
    // ==================================
    if(physEnabled)
    {
        int physTime =
        physHour * 60 + physMinute;

        int differencePhys =
        physTime - currentTime;

        if(differencePhys > 0 &&
           differencePhys < smallestDifference)
        {
            smallestDifference =
            differencePhys;

            nextClass = "PHYS211";

            nextClassHour =
            physHour;

            nextClassMinute =
            physMinute;
        }
    }

    // ==================================
    // ECON
    // ==================================
    if(econEnabled)
    {
        int econTime =
        econHour * 60 + econMinute;

        int differenceEcon =
        econTime - currentTime;

        if(differenceEcon > 0 &&
           differenceEcon < smallestDifference)
        {
            smallestDifference =
            differenceEcon;

            nextClass = "ECON";

            nextClassHour =
            econHour;

            nextClassMinute =
            econMinute;
        }
    }

    // ==================================
    // ASSEMBLY
    // ==================================
    if(assemblyEnabled)
    {
        int assemblyTime =
        assemblyHour * 60 +
        assemblyMinute;

        int differenceAssembly =
        assemblyTime - currentTime;

        if(differenceAssembly > 0 &&
           differenceAssembly <
           smallestDifference)
        {
            smallestDifference =
            differenceAssembly;

            nextClass =
            "ASSEMBLY";

            nextClassHour =
            assemblyHour;

            nextClassMinute =
            assemblyMinute;
        }
    }

    // ==================================
    // STATIC
    // ==================================
    if(staticEnabled)
    {
        int staticTime =
        staticHour * 60 +
        staticMinute;

        int differenceStatic =
        staticTime - currentTime;

        if(differenceStatic > 0 &&
           differenceStatic <
           smallestDifference)
        {
            smallestDifference =
            differenceStatic;

            nextClass =
            "STATIC";

            nextClassHour =
            staticHour;

            nextClassMinute =
            staticMinute;
        }
    }

    // -------- REMINDER --------
    if(smallestDifference <= 30 &&
      smallestDifference >= 25){
        
        showClassReminder = true;

        reminderClass = nextClass;

        reminderMinutes = smallestDifference;

        if(!reminder30Sent)
        {
            playTone(NOTE_G,120);
            delay(30);

            playTone(NOTE_C,150);
            delay(30);

            playTone(NOTE_E,250);

            reminder30Sent = true;
        }
    }
    else{
        reminder30Sent = false;
    }

    if(smallestDifference <= 15 &&
       smallestDifference >= 10)
    {
        showClassReminder = true;

        reminderClass =
        nextClass;

        reminderMinutes =
        smallestDifference;

        if(!reminder15Sent){
            playTone(NOTE_G,120);
            delay(50);

            playTone(NOTE_C,150);
            delay(50);

            playTone(NOTE_E,250);
            delay(100);

            playTone(NOTE_E,250);
            reminder15Sent = true;
        }
    }
    else{
        reminder15Sent = false;
    }

    if(smallestDifference <= 5 &&
   smallestDifference >= 0)
    {
        showClassReminder = true;

        reminderClass = nextClass;

        reminderMinutes = smallestDifference;

        if(!reminder5Sent)
        {
            playTone(NOTE_E,150);
            delay(50);

            playTone(NOTE_G,150);
            delay(50);

            playTone(NOTE_C,300);
            delay(100);

            playTone(NOTE_E,150);
            delay(50);

            playTone(NOTE_G,150);
            delay(50);

            playTone(NOTE_C,300);

            reminder5Sent = true;
        }
    }
    else
    {
        reminder5Sent = false;
    }
}

